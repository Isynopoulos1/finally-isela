...
- [ ] Portfolio API: positions, cash, total value, unrealized P&L; market-order trades (buy/sell, fractional shares, validation); value history from snapshots (every 30s and after each trade)
...
- Authentication and multi-user — single hardcoded user; `user_id` columns are a future-proofing hook only
...
- `planning/PLAN.md` §13 lists open review questions (SSE scope, price-history endpoint for the main chart, model ID check, charting library choice, watchlist `remove` action in the schema, sparkline reset on refresh) and simplification proposals. These are unresolved and should be settled during phase discussion.
